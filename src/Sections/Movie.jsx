...
const Movie = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setReleaseDate(json.release_date.split('-'));
    })();
  }, [id]);
  ...
  if (!movieDetails) {
    return (
      <div>
        <Nav searchShow={false} setSearchShow = {setSearchShow}/>
        <p className="p-10 text-center">Loading movie…</p>
      </div>
    );
  }

  const genreNames = movieDetails?.genres?.map(genre => genre.name).join(", ")
  const backdropUrl = getBackdrop(movieDetails.backdrop_path);
  ...
  async function handleClick() {
    if (!videoKey) {
      try {
        ...
        if (vid) {
          setVideoKey(vid.key);
          onPlay(vid.key, movieDetails.title);
        } else {
      ...
      onPlay(videoKey, movieDetails.title);
    }
  }
    ...
    <div >
      ...
      >
        ...
        >
          ...
          <img src={getPoster(movieDetails.poster_path)} className={' h-95vh w-[400px] rounded-xl'}/>
          <div className='text-xl'>
          <span className='flex items-center text-5xl gap-2'>
          <h1 className='font-semibold'>{movieDetails.title}
          ...
          <p>{genreNames}</p>
          <p className='font-bold mx-1 text-2xl'>·</p>
          <p>{convertMinutes(movieDetails.runtime)}</p>
          </span>
          <div className='flex items-center gap-3 mt-5'>
            <div style={{
                  background: `conic-gradient(${ratingDegree(movieDetails.vote_average)[1][0]} 0deg ${ratingDegree(movieDetails.vote_average)[0]}deg, ${ratingDegree(movieDetails.vote_average)[1][1]} ${ratingDegree(movieDetails.vote_average)[0]}deg 360deg)`
                  ...
                  {ratingFormat(movieDetails.vote_average)}
          ...
          <p className='text-gray-200 italic mt-5'>{movieDetails.tagline}</p>
